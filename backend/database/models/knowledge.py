...
class KnowledgeBase(SQLModel):
    ...
    creater: int | None = Field(default=None, sa_column=Column(Integer, nullable=True, comment="知识库创建者用户ID"))
    # 有权访问该知识库的用户ID列表（不含创建者），存为 JSON 数组
    accept_users: list[int] | None = Field(
        default_factory=list,
        sa_column=Column(JSON, nullable=False, comment="有权访问的用户ID列表（不含创建者）")
    )
    is_delete: int | None = Field(default=KnowledgeDelete.NOT.value, description="删除标识 0存在 -1删除")
...
class KnowledgeDao:
    ...
    @classmethod
    def get_by_id(cls, k_id: str, user_id: int | None = None) -> Knowledge | None:
        """
        按主键查询知识库。
    ...
    @classmethod
    def get_by_id_raw(cls, k_id: str) -> Knowledge | None:
    ...
    @classmethod
    def list(cls, user_id: int, page_num: int = 1, page_size: int = 10, name: str | None = None):
    ...
    @classmethod
    def delete(cls, k_id: str, user_id: int | None = None):
    ...
    @classmethod
    def update_accept_users(cls, k_id: str, accept_users: builtins.list[int], operator_id: int | None = None):
        """
